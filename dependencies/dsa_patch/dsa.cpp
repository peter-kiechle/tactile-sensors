...
/*!
  ...
  $Id: dsa.cpp 10351 2013-06-18 16:28:14Z Osswald2 $
...
void cDSA::QueryMatrixMask( eMatrixMaskType mask_type, MatrixMask& matrix_mask )
throw (cDSAException*)
{
    UInt8 buffer_command[2];
    buffer_command[0] = (UInt8)mask_type;
    buffer_command[1] = matrix_mask.matrix_no;
    WriteCommandWithPayload( UInt8(eDSA_READ_MATRIX_MASK), buffer_command, sizeof( buffer_command ) );

    UInt8 buffer[2 + matrix_mask.buffer_length]; // Mask + error code
    sResponse response( buffer, sizeof(buffer) );
    ReadResponse( &response, UInt8(eDSA_READ_MATRIX_MASK) );

    if ( response.size == 2 ) {
        UInt16 error_code = ((UInt16)buffer[0] << 8) | buffer[1]; // Merge bytes
        throw new cDSAException( cMsg( "Error response from DSACON32m, errorcode = %d (%s)", error_code, ErrorCodeToString( error_code ) ) );
    } else if ( response.size != sizeof( buffer ) ) {
        throw new cDSAException( cMsg( "Response with matrix mask has unexpected size %d (expected %d)", response.size, sizeof(buffer) ) );
    }


     // Copy pure mask data without leading error code
    if( mask_type == MASK_STATIC)
    {
        memcpy(&matrix_mask.buffer_static[0], buffer+2, matrix_mask.buffer_length);
    }
    else if( mask_type == MASK_DYNAMIC)
    {
        memcpy(&(matrix_mask.buffer_dynamic[0]), buffer+2, matrix_mask.buffer_length);
    }
    else 
    {
        throw new cDSAException( cMsg( "Error wrong mask type" ) );
    }
}
//-----------------------------------------------------------------

void cDSA::QueryMatrixMasks()
throw (cDSAException*)
{
    matrix_masks.clear();

    for ( uint m = 0; m < sensor_info.nb_matrices; m++ )
    {
        int nb_cells = matrix_info[m].cells_x * matrix_info[m].cells_y;
        MatrixMask matrix_mask(m, nb_cells);
        QueryMatrixMask( MASK_STATIC, matrix_mask );
        QueryMatrixMask( MASK_DYNAMIC, matrix_mask );

        matrix_masks.insert(std::make_pair(m, matrix_mask));  // Insert in map (copy)
    }
}
//----------------------------------------------------------------------
...
//-----------------------------------------------------------------
void cDSA::SetMatrixMaskCell( int m, int x, int y, bool value )
{
    int index = y * matrix_info[m].cells_x + x;
    std::map<int, MatrixMask>::iterator it = matrix_masks.find(m); // Get matrix
    int byteIndex = index / 8;
    int bitIndex = index % 8;
    std::vector<UInt8>& buffer = it->second.buffer_dynamic;
    buffer[byteIndex] = (buffer[byteIndex] & ~(1 << bitIndex)) | (value << bitIndex);
}


//-----------------------------------------------------------------
void cDSA::SetMatrixMasks( void ) 
{
    for(uint m = 0; m < sensor_info.nb_matrices; m++)
    {
        SetMatrixMask(m);
    ...
    // Print all matrix masks
//    QueryMatrixMasks();
//    for(uint m = 0; m < sensor_info.nb_matrices; m++) {
//        printf("cDSA::SetMatrixMasks: Mask %d:\n", m);
...
//-----------------------------------------------------------------
void cDSA::SetMatrixMask( int m ) 
{
    // Get matrix mask
    std::map<int, MatrixMask>::iterator it = matrix_masks.find(m); 
    MatrixMask& matrix_mask = it->second;

    // Build buffer
    UInt8 buffer_command[2 + matrix_mask.buffer_length]; // Mask + Mask type + ID
    buffer_command[0] = (UInt8)MASK_DYNAMIC;
    buffer_command[1] = matrix_mask.matrix_no;
    memcpy( &(buffer_command[2]), &(matrix_mask.buffer_dynamic[0]), sizeof( buffer_command ) );
    WriteCommandWithPayload( UInt8(eDSA_SET_DYNAMIC_MASK), buffer_command, sizeof( buffer_command ) );

    ReadAndCheckErrorResponse( "cDSA::SetMatrixMask", UInt8(eDSA_SET_DYNAMIC_MASK) );
}


//-----------------------------------------------------------------
bool cDSA::GetMatrixMaskCell( eMatrixMaskType mask_type, int m, int x, int y )
throw (cDSAException*)
{
    int index = y * matrix_info[m].cells_x + x;
    std::map<int, MatrixMask>::iterator it = matrix_masks.find(m); // Get matrix

    // Select corresponding byte and apply the bitmask that refers to the indexed cell
    bool result;
    if( mask_type == MASK_STATIC)
    {
        result = (it->second.buffer_static[index / 8] >> (index % 8)) & 1;
    }
    else if( mask_type == MASK_DYNAMIC)
    {
        result = (it->second.buffer_dynamic[index / 8] >> (index % 8)) & 1;
    }
    else 
    {
        throw new cDSAException( cMsg( "Error wrong mask type" ) );
...
//-----------------------------------------------------------------
std::string cDSA::GetMatrixMaskString( eMatrixMaskType mask_type, int m, bool pretty ) {
    // Assemble string from bitmask
    std::ostringstream ss;
    for(uint y = 0; y < matrix_info[m].cells_y; y++) {
        for(uint x = 0; x < matrix_info[m].cells_x; x++) {
            ss << GetMatrixMaskCell(mask_type, m, x, y);
        }
    }
    std::string static_mask = ss.str();
    ...
    uint pos = 0;
    int linewidth = matrix_info[m].cells_x;
    std::string formated_static_mask;
    for(uint y = 0; y < matrix_info[m].cells_y; y++) {
        formated_static_mask += static_mask.substr(pos, linewidth);
        formated_static_mask += linebreak; 
        pos += linewidth;
    }
    return formated_static_mask;
}
