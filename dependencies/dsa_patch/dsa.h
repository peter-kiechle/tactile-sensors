  ...
  \internal
    ...
    \subsection sdhlibrary_cpp_dsa_h_details SVN related, detailed file specific information:
      ...
      \par SVN file revision:
        $Id: dsa.h 10351 2013-06-18 16:28:14Z Osswald2 $
...
#include <assert.h>
#include <cmath>
#include <vector> // Matrix bitmask
#include <map> // Bitmasks of matrices
//----------------------------------------------------------------------
...
#pragma pack(pop)   // for VCC (MS Visual Studio) restore normal packing
    ...
    //!  A data structure for sensor matrix mask
    class MatrixMask
    {
    public:
        UInt8 matrix_no;
        int nb_cells;
        int buffer_length;
        std::vector<UInt8> buffer_static;  // Static bitmask
        std::vector<UInt8> buffer_dynamic; // Dynamic bitmask
        
        MatrixMask(UInt8 m, int nCells)
        {
            matrix_no = m;
            nb_cells = nCells;
            buffer_length = ceil( nb_cells / 8.0 );
            buffer_static.resize(buffer_length);
            buffer_dynamic.resize(buffer_length);
        }
    };
...
#pragma pack(pop)   // for VCC (MS Visual Studio) restore normal packing
    ...
    //! the matrix mask read from the remote DSACON32m controller
    std::map<int, MatrixMask> matrix_masks;
    ...
    */
    ...
    /*!
      Send command to request matrix masks from remote DSACON32m.
      Read and parse the response from the remote DSACON32m.
    */
    void QueryMatrixMask( eMatrixMaskType mask_type, MatrixMask& matrix_mask )
    throw (cDSAException*);


    /*!
      Query all matrix masks
    */
    void QueryMatrixMasks( void )
    throw (cDSAException*);
 ...
 public:
    ...
    */
    ...
    /*!
        Return true if connection to the DSA is open.
    */
    ...
    */
    ...
    /*!
        Set Cell <em>(x, y)</em> of dynamic matrix mask \a m
        Call SetMatrixMask(m) to finally send the new mask to the DSACON32m controller
    */
    void SetMatrixMaskCell( int m, int x, int y, bool value );

    /*!
       Send all dynamic masks to the DSACON32m controller
    */
    void SetMatrixMasks( void );

    /*!
       Send the new dynamic mask \a m to the DSACON32m controller
    */
    void SetMatrixMask( int m );

    /*!
        Return Cell <em>(x, y)</em> of matrix mask \a m
    */
    bool GetMatrixMaskCell( eMatrixMaskType mask_type, int m, int x, int y )
    throw (cDSAException*);

    /*!
        Return formated string of matrix mask \a m
    */
    std::string GetMatrixMaskString( eMatrixMaskType mask_type, int m, bool pretty = false );
